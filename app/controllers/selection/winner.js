...
module.exports = class Select {
     ...
     */
    middleware() {
        this.app.get('/winner/:id', async(req, res) => {
            try {
                ...
                jwt.verify(token, process.env.KEY_TOKEN, async(err) => {
                    ...
                    const participe = `select * from participations where id_concour = '${dec}' and final = 1`
                	let result = await db.promise().query(participe)
                	let send = []
                	for (let i = result[0].length - 1; i >= 0; i--) {
                		 let user = await axios.get(`http://localhost:4000/user/show/${result[0][i].id_etudiant}`, {
                            headers: {
